...
连接 URL 中的 `sequence` 已用于首次事件同步，客户端不需要在 WebSocket 打开后重复发送同步请求。该消息用于连接存续期间显式请求补发；小于服务端已确认序号的值不会让同步位置回退，没有新事件时服务端不会发送空批次。
...
成功操作会把仅对操作者可见的事件批次合并到确认消息中，避免额外发送一条事件消息：

```json
[2, request_id, 0, version, {"room_version":13,"first_sequence":129,"format":1,"events":[]}]
```

失败确认仍使用四项格式。对手继续通过下文的服务端事件批次接收状态变化。
...
操作确认说明请求是否被接受。成功确认的第五项包含该操作产生的事件批次，客户端不应根据操作请求自行推演结果。
